...
function levelUp(){
  ...
  document.getElementById('lu-stats').textContent=`Lv.${player.level} | 攻击+1 防御+1 生命+5`;
...
function movePlayer(dx,dy){
  ...
  if(floorCurse&&floorCurse.id==='poison_air'&&!poisonMaskActive){
    curseStepCounter++;
...
function processItems(){
  for(let i=items.length-1;i>=0;i--){
    ...
    }else if(item.type==='loreScroll'){
      // 剧情卷轴：弹出隐藏剧情文本
      spawnParticles(cx,cy,'#ffd740',10,{spread:1.2,gravity:-0.03});
      addFloatText(cx,cy-12,'📜','#ffd740');
      addMsg('拾取古老卷轴，泛着神秘的光...','special');
      const lore=pickLore();
      if(lore) showLoreScroll(lore);
...
function gotoFloor(target){
  ...
  if(!t||t<1){addMsg('[调试] 楼层无效','warning');return;}
  floor=t;
  curseStepCounter=0;
  poisonMaskActive=false;
  if(floor>=8){
  ...
  bossWarning=false;
  if(floor%5===0){bossWarning=true;}
  actionDelay=0;
  initFloor();
  addMsg(`[调试] 已跳转到第 ${floor} 层`,'special');
  saveGame();
}
