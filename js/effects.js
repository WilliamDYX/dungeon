...
function onEnemyKilled(e) {
  const goldMap={slime:[3,8],bat:[5,12],skeleton:[8,18],ghost:[10,22],demon:[25,50],lich:[15,30],mimic:[12,25],naiwa:[12,28],nailong:[20,40]};
  const g=goldMap[e.type]||[3,8];
  spawnGoldDrop(e.x, e.y, g[0], g[1]);
  if(e.elite){
    spawnGoldDrop(e.x, e.y, g[0], g[1]);
  ...
  // ★ 奶龙击杀 → 30% 概率掉落生命药剂
  if(e.type==='nailong'&&Math.random()<0.3){
    items.push({x:e.x,y:e.y,type:'hpotion',name:'生命药剂'});
    spawnParticles(e.x*TILE+TILE/2,e.y*TILE+TILE/2,'#66bb6a',10,{spread:1.2,gravity:-0.03});
    addFloatText(e.x*TILE+TILE/2,e.y*TILE-TILE/2,'🧪','#66bb6a');
  }
  // ★ 击败堕天使 → 触发逃出地牢(胜利结局)
  if(e.type==='fallenAngel'){showVictory();return}
