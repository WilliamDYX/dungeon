...
function render(){
  ...
  for(const item of items){
    ...
    switch(item.type){
      ...
      case 'loreScroll':spr=SPR.scroll;break;
      case 'weapon':spr=SPR.equipSword;break;
      case 'armor':spr=SPR.equipShield;break;
      default:continue;
    }
    ctx.drawImage(spr,sx,sy-2);
    // 剧情卷轴额外加金色脉动光晕，区分于普通卷轴
    if(item.type==='loreScroll'){
      const pulse=0.15+Math.sin(animTime*2)*0.1;
      const cx=sx+TILE/2,cy=sy-2+TILE/2;
      const g=ctx.createRadialGradient(cx,cy,0,cx,cy,TILE);
      g.addColorStop(0,`rgba(255,215,64,${pulse})`);
      g.addColorStop(1,'rgba(255,215,64,0)');
      ctx.fillStyle=g;ctx.fillRect(sx-TILE,sy-TILE-2,TILE*3,TILE*3);
    }
  ...
  for(const e of entities){
    ...
    switch(e.type){
      ...
      case 'fallenAngel':spr=SPR.fallenAngel;break;
      case 'naiwa':spr=IMG.naiwa||SPR.slime;break;
      case 'nailong':spr=IMG.nailong||SPR.slime;break;
  ...
  for(const p of projectiles){
    ...
    // 像素级弹道(箭矢)用浮点像素坐标绘制
    if(p.pixel){
      // 屏幕坐标 = 世界像素 - viewOrigin
      const sx=p.fpx-viewOriginX, sy=p.fpy-viewOriginY;
      const cx=sx, cy=sy;
      const cellX=Math.floor(p.fpx/TILE), cellY=Math.floor(p.fpy/TILE);
      if(visible[cellY]&&!visible[cellY][cellX])continue;
      ...
      for(let t=0;t<3;t++){
        ...
        const s2=5-t;
        ctx.fillRect(cx-s2/2-p.dx*(t+1)*9,cy-s2/2-p.dy*(t+1)*9,s2,s2);
      }
  ...
  ctx.fillText(`Lv ${hud.level}`,250,hpBarY+hpBarH/2);

  // ★★★ 绘制技能冷却图标 ★★★
  let sx=340;
  for(let i=0;i<skillSlots.length;i++){
    const slot=skillSlots[i];
    if(slot.unlocked&&slot.cdMax>0){
      drawCooldownIcon(sx,11,28,26,slot.cd,slot.cdMax,slot.color,slot.icon,slot.key);
      sx+=30;
    }
  }
