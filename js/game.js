...
const tutorialData=[
  ...
  },
  {
    ...
    <table class="controls-table">
      ...
      <tr><td>F</td><td>射击 - 远程攻击</td></tr>
      <tr><td>Q</td><td>旋风斩 - 近战范围攻击</td></tr>
      <tr><td>R</td><td>火球术 - 爆炸火球（2秒CD）</td></tr>
  ...
  },
  {
    ...
    content:`<div class="story-text">
      ...
      <p><span>灵魂碎片</span> - 击败Boss获得，死亡不消失，按 B 永久强化</p>
    </div>`,
    btn:'准备出发',
  },
  {
...
function spawnNPCs(){
  ...
  for(let i=0;i<count&&i<rooms.length-1;i++){
    ...
    if(type==='weaponsmith'){
      ...
      npc.name='贤者·预言';
      npc.shop=[
        {id:'skill_fire',name:'火球术卷轴',desc:'R键 - 发射爆炸火球（2秒CD）',cost:Math.floor(30*costMul),effect:()=>{playerSkills.fireball=true;addMsg('学会火球术！按 R 释放！','special');saveGame()}},
        {id:'skill_heal',name:'治愈术卷轴',desc:'T键 - 恢复50%生命（15秒CD）',cost:Math.floor(25*costMul),effect:()=>{playerSkills.heal=true;addMsg('学会治愈术！按 T 释放！','special');saveGame()}},
      ];
    }else if(type==='healer'){
      npc.name='祭司·圣光';
      npc.shop=[
...
function restartGame(){
  ...
  playerSkills={fireball:false,heal:false};
...
function saveGame(){
  ...
  try{
    const data={
      ...
      skills:{fireball:!!playerSkills.fireball,heal:!!playerSkills.heal},
...
function loadGame(){
  ...
  npcChatState={open:false,npc:null,npcType:null,messages:[],typing:false,triggeredEvents:{},discount:0,bargainCooldown:0,lastBargainResult:null};
  fireballCd=0;healCd=0;

  // 还原技能
  playerSkills={fireball:data.skills?data.skills.fireball:false,heal:data.skills?data.skills.heal:false};
